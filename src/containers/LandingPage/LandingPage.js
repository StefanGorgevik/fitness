    ...
    return (
        <div className="landing-page">
            ...
            <div className="choose-role-div" style={{ marginTop: roleChosen ? '-150px' : null }}>
                <SubmitButton name="button choose-button" click={() => chooseRoleHandler('coach')} text="Coach" />
                <SubmitButton name="button choose-button" click={() => chooseRoleHandler('trainee')} text="Trainee" />
            </div>

            <div className="auth-div">
                {role !== null &&
                    <div className="choose-auth-div" style={{ marginTop: roleChosen ? '-250px' : null, marginLeft: form && '-50px' }}>
                        <h1 className="choose-auth-h1" onClick={() => chooseAuthHandler('sign-in')} >Sign In as a {role} </h1>
                        <h1 className="choose-auth-h1" onClick={() => chooseAuthHandler('register')} >Register as a {role} </h1>
                    </div>}
